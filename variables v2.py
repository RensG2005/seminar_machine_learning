...
def fetch_compustat_data(db, start_date="2014-01-01", end_date="2025-12-31"):
    query = f"""
        SELECT
            f.gvkey,
            n.tic,
...
def fetch_crsp_monthly(db, start_date="2014-01-01", end_date="2025-12-31"):
    query = f"""
        SELECT
            permno,
            date,
...
def prepare_compustat_data(df):
    ...
    numeric_cols = ["firm_size", "roa", "leverage"]
...
def merge_ccm(comp, ccm, end_date="2025-12-31"):
    comp = comp.copy()
    ccm = ccm.copy()
    ...
    df["linkenddt"] = df["linkenddt"].fillna(pd.Timestamp('today'))
...
def winsorize_features(df, features=None, lower=0.01, upper=0.99):
    ...
    for col in features:
        ...
        df[col] = df[col].clip(lower=lo.values, upper=hi.values)
    return df
...
def vif_table(df, features):
    ...
    # force pure float array for statsmodels
    X = X.astype(float)
    ...
    return pd.DataFrame({
        "variable": features,
        "vif": np.round(vif_vals, 4)
    })

def describe_table(df, features):
    return df[features].describe().round(4)
...
def run_diagnostics(df):
    ...
    print("\n=== 2a. Describe Table ===")
    print(describe_table(df, NUMERIC_FEATURES).to_string())
    ...
    print("\n=== 3a. VIF (Multicollinearity) ===")
    print(vif_table(df, NUMERIC_FEATURES).to_string(index=False))
...
def main():
    ...
    # + one-year for 12-month rolling volatility population
    sample_start = pd.Timestamp(start_date) + pd.DateOffset(years=1)

    first_years = fetch_firm_first_year(db)
    ccm         = fetch_ccm_link(db)

    comp = (
        fetch_compustat_data(db, start_date, end_date)
        .pipe(prepare_compustat_data)
        .pipe(add_firm_age, first_years)
        .pipe(compute_book_equity)
        .pipe(merge_ccm, ccm, end_date)
    )

    crsp = (
        fetch_crsp_monthly(db, start_date, end_date)
        .pipe(prepare_crsp_data)
        .loc[lambda df: df["permno"].isin(comp["permno"].dropna().unique())]
        .copy()
    )
    ...
    comp = (
        comp
        .pipe(add_btm, crsp)
        .pipe(add_volatility, crsp, window=12, min_periods=6)
    )

    sample = (
        comp[comp["datadate"].between(sample_start, end_date)]
        .copy()
        .pipe(winsorize_features)
    )
